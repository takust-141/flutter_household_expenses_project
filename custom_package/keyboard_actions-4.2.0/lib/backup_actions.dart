...
class KeyboardActionstate extends State<KeyboardActions>
  ...
  Completer<void>? _customKeyboardPositioned;
  Completer<void>? _customKeyboardBuild;

  bool _customKeyboardUpped = false;
  ...
  void _insertOverlay() async {
    debugPrint("insert_start");
    if (_currentAction!.keyboardCustom) {
      _customKeyboardBuild = Completer<void>();
      _customKeyboardUpped = false;
    }

    _inserted = true;

    OverlayState os = Overlay.of(context, rootOverlay: true);
    ...
    _overlayEntry = OverlayEntry(builder: (context) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (_currentAction!.keyboardCustom && _customKeyboardBuild != null) {
          debugPrint("build Compleat");
          _customKeyboardBuild?.complete();
        }
      });
        ...
        children: [
          ...
          if (_currentAction!.keyboardCustom)
            AnimatedPositioned(
              ...
              left: 0,
              right: 0,
              bottom: _customKeyboardUpped
                  ? 0
                  : -(_kBarSize + _currentFooter!.preferredSize.height),
              onEnd: () {
                _customKeyboardPositioned?.complete();
                debugPrint("customanimationCompleat");
              },
              child: Material(
                color: config!.keyboardBarColor ?? Colors.grey[200],
                elevation: config!.keyboardBarElevation ?? 20,
    ...
    if (_dismissAnimationNeeded) {
      if (_currentAction!.footerBuilder == null ||
        ...
        if (mounted) {
          _overlayEntry?.markNeedsBuild();
          //カスタムキーボードがビルドされるまで待つ（重要）
          debugPrint("wait_customKeyboardBuild");
          await _customKeyboardBuild!.future;
          debugPrint("custom Keyboard Up");
          _customKeyboardUpped = true;
          _customKeyboardBuild = null;
          _updateOffset();
        }
      }
    }
    ...
    if (_dismissAnimationNeeded) {
      if (_currentFooter == null || _currentAction!.keyboardCustom == false) {
        ...
        if (mounted && !fromDispose) {
          ...
          // カスタムキーボードが閉じるまで待つ
          _customKeyboardPositioned = Completer<void>();
          _customKeyboardUpped = false;
          await _customKeyboardPositioned!.future;

          //await Future.delayed(_timeToDismiss);

          _dismissAnimation?.complete();
          _customKeyboardPositioned = null;
